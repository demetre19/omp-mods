...
| Session summary (`s.`) | Type `s.` in any OMP tab for an instant status brief — goal, progress, what needs attention — generated by the fast, cheap `mercury-2.5` diffusion model. Tab-aware: summarizes the tab you typed it in, even when tabs share a directory | [`docs/session-summary-skill.md`](./docs/session-summary-skill.md) and [`artifacts/session-summary/`](./artifacts/session-summary/) |
